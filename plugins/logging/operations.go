...
func (p *LoggerPlugin) persistRecalcOutcomes(ctx context.Context, batch []logstore.Log, outcomes []billingOutcome) (recalcTally, error) {
	...
	for i := range batch {
		...
		if cost <= 0 {
			if outcomes[i].knownZeroCost || outcomes[i].settledZero {
				costUpdates[logEntry.ID] = logstore.CostUpdate{}
				if outcomes[i].settledZero {
					tally.priced[i] = true
				}
			} else {
				tally.skipped++
		...
		if outcomes[i].debugUpdate != "" {
			// The cost and the kind's debug blob are written together so a row's
			// detail can never disagree with what it was billed. A display-only row
			// refreshes its detail alone: its cost stays NULL, so it contributes to
			// no total and priced stays false for the cursor.
